...
Na bocznych ścianach wypisany jest układ nierówności:
    \textit{2x - 5 < 9};
    \textit{x + 1 ≥ 4}.
Na płytkach mostu umieszczone są różne liczby z zakresu liczb całkowitych (np.: \textit{1}, \textit{2}, \textit{3}, \textit{4}, \textit{5}, \textit{6}, \textit{8}, \textit{7}, \textit{10}).
    \subsection{Cel zadania}
...
\end{enumerate}
\medskip
Gracz musi wybrać wyłącznie płytki z wartościami większymi lub równymi \textit{3} i mniejszymi od \textit{7}, np. \textit{3}, \textit{4}, \textit{5}, \textit{6}, \textit{8}.
\subsection{Zasady działania}
\begin{itemize}[left=1.5em, label=\textbullet, topsep=0pt, itemsep=0pt]
...
\begin{itemize}[left=1.5em, label=\textbullet, topsep=0pt, itemsep=0pt]
    \item Punkt \textit{A (1, 2)}
    \item Punkt \textit{B (3, 4)}
    \item Punkt \textit{C (5, 6)}
\end{itemize}
...
\begin{itemize}[left=1.5em, label=\textbullet, topsep=0pt, itemsep=0pt]
    \item \textit{y = 3x + 4} (czerwony)
    \item \textit{y = 5x + 8} (niebieski)
\end{itemize}
\begin{figure}[htbp]
    \centering
...
\begin{itemize}[left=1.5em, label=\textbullet, topsep=0pt, itemsep=0pt]
    ...
    \item \textit{ctg(x)}
\end{itemize}
Każdy z okręgów podzielony jest na cztery ćwiartki, oznaczone jako \textit{I}, \textit{II}, \textit{III}, \textit{IV}.
