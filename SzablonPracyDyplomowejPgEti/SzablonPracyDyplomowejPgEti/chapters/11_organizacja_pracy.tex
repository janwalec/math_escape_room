...
Organizacja pracy sprowadza się do opracowania zasad dotyczących przepływu informacji, który
ma na celu opisanie i wdrażanie zmian zachodzących w danym projekcie tak,
aby ułatwić ich zrozumienie nie tylko deweloperom, ale także użytkownikom,
którzy śledzą zmiany zachodzące w repozytorium \cite{github_about_git}.
...
\verb|Issues| (zagadnień, rys. \ref{git:issues}). Pozwalają one również na planowanie i prowadzenie dyskusji
na temat danego zagadnienia z innymi uczestnikami projektu lub użytkownikami \cite{github_about_issues}.
Dodatkową zaletą jest możliwość bezpośredniego powiązania zmian dokonanych w kodzie z~unikatowym numerem,
...
W repozytorium znajduje się tak zwana główna linia (ang. \textit{main line of development}).
...
W ten sposób można tworzyć rozgałęzienia (ang. \textit{branches}) względem głównej linii \cite{git_branches}. 
Główna gałąź (ang. \textit{main branch}), może umożliwić deweloperom zorientowanie się, czy zmiana, którą chcą wprowadzić, 
...
unikatowego identyfikatora nadawanego każdemu \verb|Issue|.

\subsection{Łączenie gałęzi}

Domyślnie gałęzie mogą być wiązane bez żadnych ograniczeń. GitHub umożliwia tworzenie zbioru zasad (ang.\textit{rulesets})
...
\verb|Pull request| (rys. \ref{git:merged}). Umożliwia on wgląd we wprowadzane modyfikacje innym uczestnikom projektu przed ich finalnym zatwierdzeniem i wdrożeniem \cite{git_about_pull_requests}.
...
unikatowy identyfikator oraz listę plików i zmian, które są z nim powiązane. Pozwala na
...
testowane (rys. \ref{kanban:screen}).
...
Kolejnym, charakterystycznym elementem metodyki Kanban jest kontrola przepływu \cite{kanban}. Dzięki sekwencyjnemu
przemieszczaniu karteczek z zadaniami na tablicy, zgodnie z zasadą, że można przesunąć tylko jedną karteczkę na raz
(od lewej do prawej), uczestnicy mają możliwość śledzenia postępu prac. Aby kontrola przepływu działała sprawnie
konieczna jest samodyscyplina uczestników w uaktualnianiu postępów wykonywanych zadań. 

Kontrola przepływu wiąże się bezpośrednio z zasadą limitu WIP (ang. \textit{Work In Progress}) określoną dla metodyki, 
