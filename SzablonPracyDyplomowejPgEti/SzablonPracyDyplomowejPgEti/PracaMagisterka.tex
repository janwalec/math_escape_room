% Wczytanie szablonu
\documentclass[nostrict]{Szablon}

\usepackage[polish]{babel}

\usepackage{placeins}
\usepackage{float}
\addbibresource{bibliography/bibliography.bib}

\titlespacing*{\subsubsection}{0pt}{0pt}{0pt}

% Definicja dokumentu
\usepackage[unicode=true]{hyperref}
\newcommand\PDFtitle{Tytuł pracy}
\newcommand\PDFauthors{Imie Nazwisko}
\hypersetup{
  pdftitle={\PDFtitle},
  pdfauthor={\PDFauthors},
}
\raggedbottom
% Zmiana czcionki dla symulacji maszynopisu (verbatim)
\makeatletter
\renewcommand{\verbatim@font}{\ttfamily\small}
\makeatother

% Część właściwa pracy
\begin{document}
\setcounter{page}{2}
\include{chapters/0a_streszczenie}
\include{chapters/0b_spis_tresci}
\include{chapters/0c_wykaz_skrotow}
\include{chapters/1_wstep}
\include{chapters/2_wprowadzenie_do_dziedziny}
\include{chapters/3_analiza_istniejacych_rozwiazan}
\include{chapters/4_projekt_systemu}
\include{chapters/5_technologie_i_narzedzia}
\include{chapters/11_organizacja_pracy}
%\include{chapters/6_analiza}
\include{chapters/9_zagadki_teoretyczne}
\include{chapters/10_zagadki_praktyczne}
\include{chapters/12_raport_z_testowania}
\include{chapters/13_sprawozdanie}
\include{chapters/7_podsumowanie}
\include{chapters/8_wykazy}
\end{document}
